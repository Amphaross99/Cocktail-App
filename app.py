...
with tab_search:
    ...
    st.info('Du kannst nach **Zutaten** oder nach dem **Namen eines Cocktails** suchen. Ohne Anführungszeichen nutzt die Suche Broad Match, z. B. `kaffee` findet auch Espresso/Kahlua und `margarita` findet auch Frozen Fruit Margarita. Mit Anführungszeichen nutzt du Exact Match, z. B. `"espresso"` oder `"Espresso Martini"`.')
    query = st.text_input("Suche", placeholder='z. B. kaffee, "vodka", margarita oder "Espresso Martini"')
    st.caption('Tipp: `"Zutat"` = Exact Match. Beispiel: `"coffee"` findet nur die exakt gepflegte Zutat Coffee. Ohne Anführungszeichen wird breiter gesucht.')
    ...
    if st.button("Cocktails suchen", type="primary"):
        ...
        if not query_terms:
            ...
            if not results:
                st.info('Keine passenden Cocktails gefunden. Tipp: Du kannst nach Zutaten oder Cocktailnamen suchen. Ohne Anführungszeichen breiter suchen, mit Anführungszeichen exakt suchen, z. B. `kaffee` oder `margarita` statt `"coffee"`.')
            else:
