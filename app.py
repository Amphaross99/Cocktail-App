...
GLASS_ICON_RULES = [
    ("rocks", "rocks", "Rocks"),
    ("old fashioned", "rocks", "Rocks"),
    ("wine", "wine", "Wine Glass"),
    ("highball", "highball", "Highball"),
    ("collins", "highball", "Highball"),
    ("champagne flute", "flute", "Champagne Flute"),
    ("flute", "flute", "Champagne Flute"),
    ("martini", "martini", "Chilled Martini/Coupe"),
    ("coupe", "martini", "Chilled Martini/Coupe"),
    ("nick and nora", "martini", "Chilled Martini/Coupe"),
...
def glass_icon_html(icon_key: str, label: str, size: int = 36) -> str:
    svgs = {
        "rocks": """
        <svg viewBox="0 0 64 64" width="{size}" height="{size}" aria-label="{label}">
          <path d="M19 12 H45 L42 54 H22 Z" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linejoin="round"/>
          <path d="M22 24 H42" fill="none" stroke="currentColor" stroke-width="1.9"/>
          <path d="M25 39 L30 34 L35 40 L40 35" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        """,
        "wine": """
        <svg viewBox="0 0 64 64" width="{size}" height="{size}" aria-label="{label}">
          <path d="M20 10 H44 C44 24 41 32 32 36 C23 32 20 24 20 10 Z" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linejoin="round"/>
          <path d="M32 36 V52" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round"/>
          <path d="M24 56 H40" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round"/>
        </svg>
        """,
        "highball": """
        <svg viewBox="0 0 64 64" width="{size}" height="{size}" aria-label="{label}">
          <path d="M22 8 H42 L40 56 H24 Z" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linejoin="round"/>
          <path d="M26 20 H38" fill="none" stroke="currentColor" stroke-width="1.9"/>
          <path d="M46 10 L37 22" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"/>
        </svg>
        """,
        "flute": """
        <svg viewBox="0 0 64 64" width="{size}" height="{size}" aria-label="{label}">
          <path d="M26 8 H38 L36 30 C35.6 35 34 38 32 40 C30 38 28.4 35 28 30 Z" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linejoin="round"/>
          <path d="M32 40 V53" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round"/>
          <path d="M25 56 H39" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round"/>
          <path d="M28.5 16 H35.5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
        </svg>
        """,
        "martini": """
        <svg viewBox="0 0 64 64" width="{size}" height="{size}" aria-label="{label}">
          <path d="M14 12 H50 L32 31 Z" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linejoin="round"/>
          <path d="M32 31 V52" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round"/>
          <path d="M24 56 H40" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round"/>
          <circle cx="37.5" cy="22.5" r="1.6" fill="currentColor"/>
        </svg>
        """,
        "shooter": """
        <svg viewBox="0 0 64 64" width="{size}" height="{size}" aria-label="{label}">
          <path d="M24 10 H40 L37 54 H27 Z" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linejoin="round"/>
          <path d="M27 22 H37" fill="none" stroke="currentColor" stroke-width="1.9"/>
        </svg>
        """,
        "default": """
        <svg viewBox="0 0 64 64" width="{size}" height="{size}" aria-label="{label}">
          <path d="M18 12 H46 L40 54 H24 Z" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linejoin="round"/>
        </svg>
        """,
    }
    svg = svgs.get(icon_key, svgs["default"]).format(size=size, label=label)
    return f"""
...
def render_recipe_card(result: dict) -> None:
    with st.container(border=True):
        glass_icon_key, glass_label = get_glass_icon(result.get("glass"))
        ice_icon, ice_label = get_ice_icon(result.get("ice"))

        top_left, top_right = st.columns([0.7, 5])
        with top_left:
            st.markdown(glass_icon_html(glass_icon_key, glass_label), unsafe_allow_html=True)
        with top_right:
            source_label = result.get("source") or "Quelle unbekannt"
            source_html = f"""
            <div style="font-size:0.78rem; opacity:0.75; margin-top:0.1rem;">Quelle</div>
            <div style="font-size:0.98rem; font-weight:600; margin-bottom:0.15rem;">{source_label}</div>
            """
            if result.get("source_url"):
                source_html += f"""<div style="font-size:0.82rem; margin-bottom:0.15rem;"><a href="{result["source_url"]}" target="_blank">Quelle öffnen</a></div>"""
            st.markdown(source_html, unsafe_allow_html=True)

        meta = [f"Glas: {result.get('glass') or glass_label}", f"{ice_icon} Ice: {ice_label}"]
        if result.get("category"):
            meta.append(f"📂 {result['category']}")
...
# Kompakte Statusanzeige statt linker Sidebar
try:
    st.markdown(
        f"""
        <div style="display:flex;flex-wrap:wrap;gap:0.45rem;margin:0.4rem 0 0.8rem 0;">
            ...
            <span class="soft-chip">★ {count_favorites()} Favoriten</span>
